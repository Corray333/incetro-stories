...
func (s *Storage) InsertBanner(project_id string, storyId string, uid int, banner types.Banner, file multipart.File, fileName string) error {
	...
	if _, err = tx.Exec(`UPDATE banners SET media_url = $1 WHERE banner_id = $2;`, "/api/images/banners/banner"+fileName, banner_id); err != nil {
		return err
	}
...
func (s *Storage) UpdateBanner(banner types.Banner, expires_at string, file multipart.File, fileName string) error {
	...
	// Update the media url of the banner
	// TODO: replace with url from .env
	if _, err = tx.Exec(`UPDATE banners SET media_url = $1 WHERE banner_id = $2;`, "/api/images/banners/banner"+fileName, banner.ID); err != nil {
		return err
	}
